set -u
# skmgr SMOKE — bring SkManager (skmgr.elf -w -k) up under FEX alongside ConfigServer.
# Goal step 1: get skmgr through RTOS init + config round-trip + creating Q_SkMgr / Q_SkMgrCtrl.
# skmgr is the softkey manager: it links libgui/libplibpp/libwinmgrlib + libSkMgrCtrl + the GMessageGui
# message libs; expect a config round-trip like Guppy (it has its own skmgrLogger.def + GData handlers).
# Reuses the run_2proc_guppy ConfigServer+X harness; closure already in /var/tmp/lr.
...
sudo rm -f /tmp/sk_cfg.log /tmp/sk.log /tmp/sk_strace.log
sudo env R="$R" SYS=/mnt/sys OEM=/mnt/plc USR=/mnt/tnc OEME=/mnt/plc EXECDIRH=/tmp/b EXECDIR=/tmp/b EXECBAT=/mnt/sys/batch/heros5 \
  ...
  HEROS_CFG_REPLY_ROUTE=1 HEROSCALL_DUMPQ="${DUMPQ:-0}" HEROSCALL_HSTRACE="${HSTRACE:-0}" DISP="$DISP" CFGPRE="$CFGPRE" SKPRE="$SKPRE" \
  SK_ARGS="$SK_ARGS" LANG=C LC_ALL=C \
  unshare -m bash -c '
    set -u; ulimit -c 0
    ...
    ( env HEROS_FAKE_NS=1 HEROSCALL_VERBOSE=1 MALLOC_ARENA_MAX=1 GLIBC_TUNABLES=glibc.malloc.arena_max=1 \
        LD_PRELOAD="$CFGPRE" timeout -s KILL 240 FEXInterpreter "$R/heros5/bin/ConfigServer.elf" \
        -p=~/cfgserver cfgserver -f=/mnt/sys/config/jhconfigfiles.cfg -i=Nc > /tmp/sk_cfg.log 2>&1 ) &
    CFGPID=$!
    i=0; while [ $i -lt 200 ]; do
      grep -qE "HWS stub: replied|RUNUP_COMPLETE" /tmp/sk_cfg.log 2>/dev/null && { echo "  ConfigServer run-up at ${i}*0.5s"; break; }
      sleep 0.5; i=$((i+1)); done
    sleep 8
    echo "### skmgr (-p=~/skmgr skmgr $SK_ARGS, DISPLAY=$DISP) ###"
    timeout -s KILL "${SK_TIMEOUT:-120}" /usr/bin/strace -f -qq -e trace=openat,connect,writev -o /tmp/sk_strace.log \
      env HEROSCALL_VERBOSE="${SK_VERBOSE:-1}" HEROSCALL_HSTRACE="${HSTRACE:-0}" MALLOC_ARENA_MAX=1 GLIBC_TUNABLES=glibc.malloc.arena_max=1 LD_PRELOAD="$SKPRE" \
      FEXInterpreter "$R/heros5/bin/skmgr.elf" -p=~/skmgr skmgr $SK_ARGS > /tmp/sk.log 2>&1 || true
    echo "### skmgr done ###"
...
echo ""
echo "=== skmgr smoke result ==="
echo "  ConfigServer serve-loop: $(sudo grep -acE "Ev_receive .*0101100" /tmp/sk_cfg.log 2>/dev/null)"
echo "  skmgr lines: $(sudo wc -l </tmp/sk.log 2>/dev/null)  crash(sig6/11): $(sudo grep -acE "signal 6|signal 11|terminating signal" /tmp/sk.log 2>/dev/null)"
echo "  Q_create by skmgr (queue names): $(sudo grep -aoE "Q_create [^ ]+ .*0x3[0-9a-f]+" /tmp/sk.log 2>/dev/null | grep -aoE "\"[^\"]+\"" | sort -u | tr '\n' ' ')"
echo "  Q_SkMgr / Q_SkMgrCtrl created: $(sudo grep -acE "Q_SkMgr" /tmp/sk.log 2>/dev/null)"
echo "  config round-trip (Q_send->cfg / reads): $(sudo grep -acE "CfgServerQueue|Q_ident .CfgServer" /tmp/sk.log 2>/dev/null)"
echo "  X11 connect: $(sudo grep -acE "X11-unix" /tmp/sk_strace.log 2>/dev/null)"
echo "  --- skmgr stderr/log tail (non-RTOS) ---"
sudo grep -avE "^\[t[0-9]+ hc|^\[rtos\]|BUS_ADRALN|unaligned atomic" /tmp/sk.log 2>/dev/null | tail -30
echo "  --- skmgr last RTOS lines ---"
sudo grep -aE "^\[t[0-9]+ hc|^\[rtos\]|Q_create|Q_ident" /tmp/sk.log 2>/dev/null | tail -25
echo "logs: /tmp/sk_cfg.log /tmp/sk.log /tmp/sk_strace.log"
