...
$CC -shared -fPIC -O2 -o "$R/lib/linebuf.so" "$EMU/linebuf.c" 2>&1 | sed "s/^/  linebuf: /"   # LINEBUF (default ON): line-buffer stdout so a SIGKILLed child's few hundred bytes of log are not lost
...
timeout -s KILL $APPSTART_TIMEOUT /usr/bin/strace -f -qq -e trace=execve,connect,clone,clone3,fork,vfork,newfstatat,statx,access,faccessat,faccessat2,stat -o /tmp/a_strace.log \
  ...
  HEROSCALL_INJECT_PROM_HIDE=${HEROSCALL_INJECT_PROM_HIDE:-0} \
  HEROSCALL_INJECT_PROM_ACTIVATE=${HEROSCALL_INJECT_PROM_ACTIVATE:-0} \
  ${HEROSCALL_PROM_ACTIVATE_DELAY:+HEROSCALL_PROM_ACTIVATE_DELAY=$HEROSCALL_PROM_ACTIVATE_DELAY} \
  ${HEROSCALL_PROM_ACT_QUEUE:+HEROSCALL_PROM_ACT_QUEUE=$HEROSCALL_PROM_ACT_QUEUE} \
  ${HEROSCALL_PROM_ACT_REMAP:+HEROSCALL_PROM_ACT_REMAP=$HEROSCALL_PROM_ACT_REMAP} \
  ${HEROSCALL_PROM_ACT_REMAP_DELAY:+HEROSCALL_PROM_ACT_REMAP_DELAY=$HEROSCALL_PROM_ACT_REMAP_DELAY} \
  ${HEROSCALL_PROM_ACT_REMAP_HOLD:+HEROSCALL_PROM_ACT_REMAP_HOLD=$HEROSCALL_PROM_ACT_REMAP_HOLD} \
  HEROSCALL_WMACT_SELECT=${HEROSCALL_WMACT_SELECT:-0} \
  HEROSCALL_WMACT_SCREEN=${HEROSCALL_WMACT_SCREEN:-1} \
  HEROSCALL_WMACT_ALT_SCREEN=${HEROSCALL_WMACT_ALT_SCREEN:-0} \
  HEROSCALL_WMACT_ONCE=${HEROSCALL_WMACT_ONCE:-0} \
  HEROSCALL_WMACT_DELAY=${HEROSCALL_WMACT_DELAY:-180} \
  ...
  LINEBUF=${LINEBUF:-1} \
  LD_PRELOAD=$([ "${LINEBUF:-1}" != 0 ] && printf '/lib/linebuf.so:')$([ "${HEU_GRANT:-0}" = 1 ] && printf '/lib/heurights.so:')$([ "${CXATHROW:-0}" = 1 ] && printf '/lib/cxathrow.so:')$([ "${FREDFREE:-0}" = 1 ] && printf '/lib/fredfree.so:')$([ "${FMDEL:-0}" = 1 ] && printf '/lib/fmdel.so:')$([ "${FKEEPVEC:-0}" = 1 ] && printf '/lib/fkeepvec.so:')/lib/arena_stub.so:/lib/herosapi_shim.so:/lib/heros_rtos.so \
