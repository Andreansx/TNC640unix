set -u
# Guppy SCOUT — the REAL main TNC-screen MMI (GTK2 + Python2.7, WndFullScreen::Create), NOT HrMmi.
# HrMmi.elf is the HANDWHEEL-unit MMI (PLib, HR420/520/550 display); Guppy.elf is the `~/mmi`/`~/Guppy`
# subsystem = the main fullscreen screen the operator sees. Closure already lives in /var/tmp/lr (the full
# HeROS /usr/lib overlay). This reuses the run_2proc_hrmmi ConfigServer+X harness and just launches Guppy
# with its batch args (-R=UnloadOEM -v=c -i=Nc -s=Sim) to pin Guppy's FIRST FEX-native blocker.
...
GUPPY_ARGS="${GUPPY_ARGS:--R=UnloadOEM -v=c -i=Nc -s=Sim}"
...
sudo env R="$R" SYS=/mnt/sys OEM=/mnt/plc USR=/mnt/tnc OEME=/mnt/plc EXECDIRH=/tmp/b EXECDIR=/tmp/b EXECBAT=/mnt/sys/batch/heros5 \
  ...
  HEROS_CFG_REPLY_ROUTE=1 HEROSCALL_DUMPQ="${DUMPQ:-0}" DISP="$DISP" CFGPRE="$CFGPRE" MMIPRE="$MMIPRE" \
  GUPPY_BIN="$GUPPY_BIN" GUPPY_ARGS="$GUPPY_ARGS" LANG=C LC_ALL=C \
  unshare -m bash -c '
    set -u; ulimit -c 0
    ...
    echo "  ConfigServer serve-loop: $(grep -acE "Ev_receive .*0101100" /tmp/g_cfg.log 2>/dev/null)"
    sleep 8
    echo "### Guppy ($GUPPY_BIN $GUPPY_ARGS, DISPLAY=$DISP) ###"
    ( sleep 150; rm -f /tmp/g_screen.xwd; DISPLAY=$DISP xwd -root -out /tmp/g_screen.xwd 2>/dev/null && echo "  screenshot bytes: $(wc -c </tmp/g_screen.xwd 2>/dev/null)" ) &
    timeout -s KILL "${MMI_TIMEOUT:-180}" /usr/bin/strace -f -qq -e trace=openat,connect,writev -o /tmp/g_strace.log \
...
echo ""
echo "=== Guppy scout result ==="
echo "  ConfigServer serve-loop: $(sudo grep -acE "Ev_receive .*0101100" /tmp/g_cfg.log 2>/dev/null)  broadcasts: $(sudo grep -acE "Q_send -> queue 0x307" /tmp/g_cfg.log 2>/dev/null)"
...
echo "  GTK/window markers: $(sudo grep -acE "gtk|Gtk|WndFullScreen|XCreateWindow|gdk" /tmp/g_mmi.log 2>/dev/null)"
echo "  screenshot unique colours (1=blank): $(DISPLAY=$DISP convert /tmp/g_screen.xwd -format "%k" info: 2>/dev/null || echo n/a)"
echo "  Guppy stderr tail:"; sudo grep -avE "^\[t[0-9]+ hc|^\[rtos\]" /tmp/g_mmi.log 2>/dev/null | tail -20
echo "  Guppy last RTOS:"; sudo grep -aE "^\[t[0-9]+ hc|^\[rtos\]" /tmp/g_mmi.log 2>/dev/null | tail -8
echo "logs: /tmp/g_cfg.log /tmp/g_mmi.log /tmp/g_strace.log"
