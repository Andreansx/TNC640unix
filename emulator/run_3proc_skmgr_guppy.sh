...
if [ "$USE_XVFB" = "1" ]; then
  ...
  # Stage the WM activate/select wires (byte-exact, originally emitted by the genuine libGMessageGui
  # serializer via scratchpad/gen_wm_wires.sh) so INJECT_WMGR_ACTIVATE+WMACT_SELECT works on a fresh /tmp.
...
sudo env R="$R" SYS=/mnt/sys OEM=/mnt/plc USR=/mnt/tnc OEME=/mnt/plc EXECDIRH=/tmp/b EXECDIR=/tmp/b EXECBAT=/mnt/sys/batch/heros5 \
  ...
  HEROSCALL_INJECT_WMGR_ACK="${INJECT_WMGR_ACK:-0}" EMPTYPOLL_DIAG="${EMPTYPOLL_DIAG:-0}" EMPTYPOLL_YIELD="${EMPTYPOLL_YIELD:-0}" WMGR_SCREEN="${WMGR_SCREEN:-0}" WMQ_BREAK="${WMQ_BREAK:-1}" WMQ_BREAK_N="${WMQ_BREAK_N:-2000}" \
  HEROSCALL_INJECT_WMGR_TIMER="${INJECT_WMGR_TIMER:-0}" HEROSCALL_WMGR_TIMER_WARMUP_MS="${WMGR_TIMER_WARMUP_MS:-2500}" HEROSCALL_WMGR_TIMER_TICK_MS="${WMGR_TIMER_TICK_MS:-55}" \
  ...
  HEROSCALL_INJECT_SK_ACTIVATE="${INJECT_SK_ACTIVATE:-0}" HEROSCALL_SK_ACT_THRESH="${SK_ACT_THRESH:-5}" HEROSCALL_SK_ACT_SCREEN="${SK_ACT_SCREEN:-}" HEROSCALL_SK_ACT_GROUP="${SK_ACT_GROUP:-0}" HEROSCALL_SK_ACT_HANDLE="${SK_ACT_HANDLE:-13}" HEROSCALL_SK_ACT_BOOL="${SK_ACT_BOOL:-0}" HEROSCALL_SK_ACT_LOGIN_SETTLE="${SK_ACT_LOGIN_SETTLE:-2}" HEROSCALL_SK_ACT_QUEUE="${SK_ACT_QUEUE:-}" \
  ...
  HWVIEWER_SK_USAGE="${HWVIEWER_SK_USAGE:-}" HWVIEWER_GEOMETRY="${HWVIEWER_GEOMETRY:-}" WINMGR="${WINMGR:-0}" WM_LAYOUT="${WM_LAYOUT:-}" WM_SIZE="${WM_SIZE:-}" WM_VERBOSE="${WM_VERBOSE:-1}" WM_SEGVBT="${WM_SEGVBT:-0}" WM_BTRACE="${WM_BTRACE:-0}" WM_STRACE_TRACE="${WM_STRACE_TRACE:-connect,writev}" WM_SEM_INIT="${WM_SEM_INIT:-0}" WM_SEM_FORCE_OK="${WM_SEM_FORCE_OK:-4000}" WM_PNAME="${WM_PNAME:-1}" SYSFIRE="${SYSFIRE:-0}" SYSFIRE_MASK="${SYSFIRE_MASK:-00ff0000}" WM_FIRE_LIMIT="${WM_FIRE_LIMIT:-0}" WM_FIRE_YIELD="${WM_FIRE_YIELD:-0}" \
  GUPPY_BIN="$GUPPY_BIN" GUPPY_ARGS="$GUPPY_ARGS" GUPPY_C="$GUPPY_C" GUPPY_PNAME="${GUPPY_PNAME:-0}" SK_ARGS="$SK_ARGS" GUPPY_DISPLAY="${GUPPY_DISPLAY:-}" HWV_FORCE_FS="${HWV_FORCE_FS:-}" SHOT_AT="${SHOT_AT:-150}" WININSPECT="${WININSPECT:-0}" MAPFORCE="${MAPFORCE:-}" WMPOKE="${WMPOKE:-}" WMSHOW="${WMSHOW:-}" WMFLOAT="${WMFLOAT:-}" BARCOPY="${BARCOPY:-}" BARCOPY_PER="${BARCOPY_PER:-150}" LANG=C LC_ALL=C \
  unshare -m bash -c '
    set -u; ulimit -c 0
    ...
    timeout -s KILL "${MMI_TIMEOUT:-200}" /usr/bin/strace -f -qq -e trace=openat,connect,writev -o /tmp/g_strace.log \
      env DISPLAY="$GDISP" HEROSCALL_VERBOSE="${MMI_VERBOSE:-1}" HEROSCALL_HSTRACE="${HSTRACE:-0}" MALLOC_ARENA_MAX=1 GLIBC_TUNABLES=glibc.malloc.arena_max=1 PYTHONHOME=/usr NO_NCK_WINMGR="$NO_NCK_WINMGR" WMFORCE="$WMFORCE" SKFORCE="$SKFORCE" HWVIEWER_SK_USAGE="$HWVIEWER_SK_USAGE" HWVIEWER_GEOMETRY="$HWVIEWER_GEOMETRY" HEROSCALL_WMQ_BREAK="${WMQ_BREAK:-1}" HEROSCALL_WMQ_BREAK_N="${WMQ_BREAK_N:-2000}" HEROSCALL_EMPTYPOLL_YIELD="${EMPTYPOLL_YIELD:-0}" HEROSCALL_WMGR_ROOT="${WMGR_ROOT:-0}" HEROSCALL_WMGR_SCREEN="${WMGR_SCREEN:-0}" HWV_FORCE_FS="${HWV_FORCE_FS:-}" HEROSCALL_EV_NOWAIT_YIELD="${EV_NOWAIT_YIELD:-0}" HEROSCALL_EV_NOWAIT_YIELD_N="${EV_NOWAIT_YIELD_N:-64}" HEROSCALL_EV_INJECT_WANT="${G_EVW:-0}" HEROSCALL_EV_INJECT_BIT="${G_EVB:-0}" HEROSCALL_EV_UNBLOCK_MS="${G_EVMS:-0}" HEROSCALL_PNAME="${GUPPY_PNAME:-0}" LD_PRELOAD="$MMIPRE" \
