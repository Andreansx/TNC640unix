...
sudo env R="$R" SYS=/mnt/sys OEM=/mnt/plc USR=/mnt/tnc OEME=/mnt/plc EXECDIRH=/tmp/b EXECDIR=/tmp/b EXECBAT=/mnt/sys/batch/heros5 \
  ...
  HEROS_CFG_REPLY_ROUTE=1 HEROS_EV_SIGWAKE="${EV_SIGWAKE:-0}" HEROSCALL_PIDENT_SELF="${PIDENT_SELF:-1}" HEROSCALL_SK_REPLY_FORCE="${SK_REPLY_FORCE:-1}" HEROSCALL_DUMPQ="${DUMPQ:-0}" HEROSCALL_HSTRACE="${HSTRACE:-0}" DISP="$DISP" \
  HEROSCALL_INJECT_SK_ACTIVATE="${INJECT_SK_ACTIVATE:-0}" HEROSCALL_SK_ACT_THRESH="${SK_ACT_THRESH:-5}" HEROSCALL_SK_ACT_SCREEN="${SK_ACT_SCREEN:-}" HEROSCALL_SK_ACT_GROUP="${SK_ACT_GROUP:-0}" HEROSCALL_SK_ACT_HANDLE="${SK_ACT_HANDLE:-13}" \
  HEROSCALL_INJECT_SK_FLOW="${INJECT_SK_FLOW:-0}" HEROSCALL_INJECT_AREA_ACK="${INJECT_AREA_ACK:-0}" \
