...
static int prom_activate=-1; /* HEROSCALL_INJECT_PROM_ACTIVATE=1: PATH B — deliver PromActivateNotifyMsg to Fred (Gate 2) */
...
/* INJECT_PROM_ACTIVATE (HEROSCALL_INJECT_PROM_ACTIVATE=1) — GATE 2, PATH B (route around the dead prom).
 * promview.elf CRASHES at init (terminate: PciHardware::Exception) before its QProMRequest serve loop, so it
 * never replies to the editor's activation request -> Fred's FControl activation-state (member85) never reaches
 * 3 -> Fred never sends its own real SkMgrActivate -> no softkey bar. On a real boot prom would answer the
 * editor's registration by sending it a PromActivateNotifyMsg (type 0x404703E0) on the editor's OWN FControl
 * queue (Fred advertised "Ed/mmi.ProgEditQueue" in its QProMRequest registration payload). We deliver THAT
 * EXACT message -- byte-exact from the genuine libGMessageGui serializer (default PromActivateNotifyMsg:
 * body [0x404703E0][screen=0][state=0], 12 bytes) -- to ProgEditQueue. Fred's FControl::DispatchMessage@0x89b10
 * routes *(msg+4)&0x7FFFFFFF==0x404703E0 -> FControl::OnProMActivateNotify@0x891a0, which (state!=3) sets
 * member85=3 and calls FControlCallbackTarget::RequestClientFocus. Because member136 (this+544, prom's assigned
 * registration id) is still its -1 init sentinel (prom died before Fred registered), OnProMActivateNotify SKIPS
 * the PromActivationAckMsg back to prom -- and even if it didn't, that ack lands on a dead queue (harmless). The
 * winmgr focus-grant (WMACT editor-foreground) then turns RequestClientFocus -> CMD_ONACTIVATE(7) -> DoActivationCmd
 * -> FSoftkeyView::ActivateApp -> ... -> Fred's REAL SkMgrActivate(0x028a0200) -> skmgr draws the bar. This is the
 * genuine prom-less activation path (FControl::ActivateMyApp's self-synth branch delivers the same message
 * in-process when member136==-1); no fabricated SkMgrActivate. Byte-exact, virtiofs-immune (inline). */
static void post_prom_activate_notify(void){
    const char* qn=getenv("HEROSCALL_PROM_ACT_QUEUE"); if(!qn||!*qn) qn="ProgEditQueue";
    lock(); int q=q_find_slot(qn); uint32_t qid=(q>=0)?C->queues[q].id:0; unlock();
    if(!qid){ fprintf(stderr,"[promact] %s not up yet\n",qn); return; }
    unsigned char w[12]; put32(w+0,0x404703E0u); put32(w+4,0u); put32(w+8,0u);
    fprintf(stderr,"[promact] post PromActivateNotifyMsg(0x404703E0) -> %s 0x%x (PATH B: Fred FControl activation)\n",qn,qid);
    q_send(qid,w,sizeof w,0);
}
...
static void* wm_activate_thread_fn(void* arg){
    ...
    /* PATH B (INJECT_PROM_ACTIVATE): after the editor screen is foregrounded (WMACT above), deliver the exact
     * PromActivateNotifyMsg prom would have sent Fred -> FControl activation-state 3 -> RequestClientFocus ->
     * (winmgr grant) CMD_ONACTIVATE -> real SkMgrActivate -> bar. Posted once (same single-poster CAS), after a
     * settle gap so winmgr has processed the foreground switch and Fred's FControl serve loop is reading
     * ProgEditQueue. Retries until the queue appears. */
    if(prom_activate){
        if(!claimed){
            if(__atomic_exchange_n(&C->wm_activate_posted,1,__ATOMIC_ACQ_REL)!=0) return NULL; /* another won */
            claimed=1;
        }
        long pd=5; const char* e=getenv("HEROSCALL_PROM_ACTIVATE_DELAY");
        if(e){ pd=0; for(const char*p=e;*p>='0'&&*p<='9';p++) pd=pd*10+(*p-'0'); }
        { struct timespec s={pd,0}; nanosleep(&s,0); }   /* let winmgr settle the foreground before activation */
        const char* qn=getenv("HEROSCALL_PROM_ACT_QUEUE"); if(!qn||!*qn) qn="ProgEditQueue";
        for(int t=0;t<180;t++){
            lock(); int q=q_find_slot(qn); int up=(q>=0); unlock();
            if(up){ post_prom_activate_notify(); break; }
...
static void wm_activate_spawn(void){
    ...
    if(prom_activate<0){ const char*e=getenv("HEROSCALL_INJECT_PROM_ACTIVATE"); prom_activate=e&&e[0]=='1'; }
    if(!wm_activate && !prom_hide && !prom_activate){ spawned=1; return; }
