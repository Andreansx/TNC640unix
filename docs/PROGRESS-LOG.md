# TNC640unix — progress log (full historical tracker)

> Archived from CLAUDE.md on 2026-07-06 to keep CLAUDE.md lean and fast to load.
> This is the COMPLETE session-by-session frontier log, the blocker-chain (#1–#6)
> narrative, the 335-object binary inventory, the recompiled-function verification
> tables, harvested credentials/recipes, and the deprioritized Track A / x86_64
> material. **Live, durable instructions are in CLAUDE.md** — consult this file for
> the history behind a decision or the exact recipe for a past fix. NOT auto-loaded
> into context (that is the point of the split).
...
> ## ★★★★★★ SOFTKEY WIRE ENCODING SOLVED (the 0x78 is GONE) via a DETERMINISTIC GMessage serializer — the bar gate MOVES from the wire format to the SkMgrGMsgController `this+5` handler-precondition (2026-06-27)
> The brief's repeatedly-documented "ONLY remaining gate" — *crack the exact wire format of the 3 SkMgr
> messages (Login/SetMenu/Activate) so they deserialize past GMessage::Read's GMsgException 0x78* — is
> **SOLVED + VERIFIED**. No more schema-table guessing.
> **HOW (`scratchpad/build_setmenu.c`, the deterministic serializer):** dlopen libgmsglib + libGMessageGui,
> construct the REAL SkMgr message (`_ZN10SkMgrLoginC1Ev` / `_ZN12SkMgrSetMenuC1Ev` / `_ZN13SkMgrActivateC1Ev`
> + operator->), set fields via the genuine `GMsgString::SetValue@0x30850` / direct body writes, then run the
> GENUINE serializer: `GMsgOutStream(GMessage&)@0x57060` hardcodes TEXT mode (`a3=true` → `GMsgStreamBase+6`),
> so for BINARY = construct the stream, **flip `GMsgStreamBase+6` to 0**, `GMsgStreamBase::allocate@0x59620`,
> `GMessage::Write@0x40d30`, read the wire at `base+52`(ptr)/`base+60`(size). The encoding it emits (now the
> ground truth, replaces the .rodata-schema guesses): present GMsgUnsigned/Bool = `[code][value]`, ABSENT =
> `[code|0x80000000]` (no value); present enum (SkMgrSoftkeyScreen 0x028a006b / Group 0x028a004b) =
> `[code][value]`; String = `[0xe7][len][bytes]`.
> **DELIVERY (`emulator/heros_rtos.c`):** `inject_sk_flow` now LOADS the 3 byte-exact wire files
> (`${HEROSCALL_SK_FLOW_DIR:-/tmp}/sk_{login,setmenu,activate}.bin`, generated by `scratchpad/gen_sk_wires.sh`)
> via `post_sk_wire_file` and q_sends them IN ORDER to **Q_SkMgr** (by name; the [skflow] timer thread, bypassing
> Guppy's GData wedge). Falls back to the inline schema-guessed wires only if the files are absent. Also corrected
> the inline `INJECT_SK_ACTIVATE` wire to the same absent/present encoding (+`HEROSCALL_SK_ACT_BOOL` knob).
> **VERIFIED (`scratchpad/sktest.sh`, byte-exact wires):** all 3 posted (`posted all 3 byte-exact wire files`),
> **skmgr READS ALL 3** (`skmgr_reads_flow_0x314: 3`; previously it wedged on the malformed SetMenu and never
> read the Activate), **0 GMsgException 0x78, 0 crash**. The deserialization works END-TO-END — the malformed-wire
> blocker that gated the entire INJECT_SK_FLOW path is GONE. (A/B: the old inline wires → 0x78 / wedge.)
> ★ BUT the bar still does NOT draw (no PFrame, no .spj open, 0 PutImage, screenshot 118 colours). **NEW GATE,
> precisely pinned (idalib on skmgr.elf) — and it REFUTES the brief's premise "login+SetMenu set this+5":** all
> three softkey handlers — **`SkMgrGMsgController::OnLogin@0x58590`, `OnSetMenu@0x58c70`, `OnActivation@0x5a5a0`**
> — open with `v3 = *(this+5); if (v3 != 0){…} return;` — they **READ `this+5` and RETURN EARLY (no-op) when it
> is 0; NONE of them SET it**. `this+5` (the controller's GData request-data) is created ONLY by
> **`SkMgrGMsgController::NcCtrlInitCreate@0x5ca70`** (`v1 = new SKManagerGDataSkMgrRequestData(0x1260);
> *(this+5) = v1; GData::RegisterInstance(v1)`). So the byte-exact messages deserialize cleanly but are no-ops
> because the controller's `this+5` GData request-data was not live when they arrived. ⇒ the bar is now gated on
> **the SkMgrGMsgController NcCtrl-init / `this+5` precondition** (a handler-readiness gate), NOT the wire format
> (solved) and NOT the queue (inject targets Q_SkMgr correctly). The handler reads body+20/+32/+44/+56/+68 +
> the GMsgString(.spj)@+72 then `GData::Notify` → `SkMgrFrame::OnActivation@0x42170` → SkMgrScreen::Activate →
> PFrame → BuildSoftkeyBar → PutImage; that whole chain is downstream of the `this+5` gate.
> **NEXT:** (a) find `NcCtrlInitCreate`'s caller — when/whether skmgr sets `this+5` in this 4-proc run (likely
> the NcCtrlModule init / the WM-screen setup); (b) probe `this+5` live (is it 0 at inject time? a timing gate vs
...
## Migrated from CLAUDE.md (2026-07-12): superseded "Prior frontier" blocks (2026-07-11, 2026-07-06)

These two frontier narratives were resident in CLAUDE.md every session; the current frontier (2026-07-12) supersedes them. Moved here to keep CLAUDE.md lean, per this file's own header policy.
...
## Migrated from CLAUDE.md (2026-08-13): superseded frontier blocks (2026-07-12 … 2026-07-15)
These were the live frontier write-ups until the 2026-07-29 crossing and the 2026-08-13 emulator
fixes superseded them. Kept verbatim for the history behind each decision.
